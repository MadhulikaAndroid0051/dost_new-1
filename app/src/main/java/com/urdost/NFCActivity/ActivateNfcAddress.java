...
public class ActivateNfcAddress extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.digital_bussinos_card_activity);
        ButterKnife.bind(this);
        tvTitle.setText("Dost Business card");
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.btn_search:
                ...
                cvQuantity.setVisibility(View.VISIBLE);
               // getBalance();
                if (validate()) {
    ...
    private void PayNow() {
        ...
        object.addProperty("NoOfSeats", bundle.getString("NoOfSeet"));
        object.addProperty("Pk_NfcProfileId", PreferencesManager.getInstance(context).getPk_userId());
        object.addProperty("TotalPrice", bundle.getString("price"));
        object.addProperty("PK_EventId",tvEventid);
        object.addProperty("PlanId",  tvEventid);
        object.addProperty("ReferralBV", bundle.getString("ReferBv"));
        object.addProperty("TeamBV", bundle.getString("TeamBv"));
        object.addProperty("DeliveryCharge", tvHandlingCharge.getText().toString().trim());
        object.addProperty("TotalPayable", etWalletBalance.getText().toString().trim());
        object.addProperty("Gst", tvGst.getText().toString().trim());
        object.addProperty("Dwallet", etWalletBalance.getText().toString().trim());
        object.addProperty("Fwallet", "0.00");
        object.addProperty("Discount", discount);
        //  object.addProperty("AddressId", "3");
        object.addProperty("PinCode", etPincode.getText().toString().trim());
        object.addProperty("City", etCity.getText().toString().trim());
        object.addProperty("State", etState.getText().toString().trim());
        object.addProperty("Address", etAddress.getText().toString().trim());
        object.addProperty("AddedBy", PreferencesManager.getInstance(context).getPk_userId());
        Call<ResponseNFCPayNow> call = apiServices.getNfcPayNow(object);
        call.enqueue(new Callback<ResponseNFCPayNow>() {
            @Override
            public void onResponse(Call<ResponseNFCPayNow> call, Response<ResponseNFCPayNow> response) {
                hideLoading();
                LoggerUtil.logItem(response.body());
                if (response.body().getStatusCode().equalsIgnoreCase("200")) {
...
////
            ...
            @Override
            public void onFailure(Call<ResponseNFCPayNow> call, Throwable t) {
                hideLoading();
            }
        });
    }
